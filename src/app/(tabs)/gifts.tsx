...
const GIFT_STORE = [
  { id: 1, name: '🌹 گل رز',      type: 'ساده',     coins: 10 },
  { id: 2, name: '💐 دسته گل',    type: 'خفن',      coins: 50 },
  { id: 3, name: '💎 الماس',      type: 'ویژه',     coins: 200 },
  { id: 4, name: '👑 تاج طلایی',  type: 'افسانه‌ای', coins: 500 },
  { id: 5, name: '☕ قهوه',        type: 'ساده',     coins: 15 },
  { id: 6, name: '🎭 ماسک',       type: 'خفن',      coins: 80 },
];

const RECEIVED_GIFTS = [
  { id: 1, from: 'کیان',   gift: '💎 الماس',    date: 'امروز ۱۴:۲۲',  pinned: true },
  { id: 2, from: 'آرمان',  gift: '🌹 گل رز',    date: 'دیروز' },
];
        ...
        <View style={styles.giftGrid}>
          {GIFT_STORE.map(g => (
            <TouchableOpacity key={g.id} style={styles.storeCard}>
              <Text style={styles.storeEmoji}>{g.name.split(' ')[0]}</Text>
              <Text style={styles.storeName}>{g.name.split(' ').slice(1).join(' ')}</Text>
...
const styles = StyleSheet.create({
  ...
  wallet: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  ...
  pinnedBadge: { fontSize: 10, color: Colors.goldDeep, fontFamily: Fonts.semiBold },
  giftFrom: { fontSize: 11.5, color: Colors.muted, fontFamily: Fonts.regular, marginTop: 2 },
  giftAction: { padding: 6 },
  giftActionTxt: { fontSize: 20, color: Colors.muted },
